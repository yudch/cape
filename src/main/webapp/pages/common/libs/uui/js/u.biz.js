		...
		initialize: function(element, options, viewModel) {
			...
			for(var key in events){
				if (this.isDomEvent(key)){
					this.addDomEvent(key, $.getFunction(events[key]))
				}
				else{
					this.on(key, $.getFunction(events[key]))
				}
			}	
		},
		/**
...
//	}
	 ...
	 * @param {Object} index
	 */
	DataTable.fn.setRowFocus = function(index){
		var rowId = null
		if(index instanceof Row) {
			index = this.getIndexByRowId(index.rowId)
			rowId = index.rowId
	 ...
	 *设置row中某一列的值 
	 */
	Row.fn.setValue = function(fieldName, value, ctx){
...
+function($){
	...
	var Grid = function(element, options, viewModel,app) {
		...
		$("div",this.$element).each(function() {
			...
			if(eType == '')
				eType = 'string';
			if(eType == 'string' || eType == 'checkbox' || eType == 'combo' || eType == 'radio' || eType == 'float' || eType == 'datetime'|| eType == 'date'){
				if(eType == 'string'){
					var compDiv = $('<input type="text" style="width:100%;margin:0px;min-height:20px;font-size:12px;color:#444">');
					var comp = new $.compManager.plugs.string(compDiv[0],eOptions,viewModel);
					
				}else if(eType == 'checkbox'){
					var compDiv = $('<input id="' + oThis.id + "_edit_field_" + column['field'] + '" type="checkbox">');
					var comp = new $.compManager.plugs.checkbox(compDiv[0],eOptions,viewModel);
					
				}else if(eType == 'combo'){
					var compDiv = $('<div>');
					var comp = new $.compManager.plugs.combo(compDiv[0],eOptions,viewModel);
					
				}else if(eType == 'radio'){
					var compDiv = $('<input id="' + oThis.id + "_edit_field_" + column['field'] + '" type="text">');
					var comp = new $.compManager.plugs.radio(compDiv[0],eOptions,viewModel);
				}else if(eType == 'float'){
					var compDiv = $('<input id="' + oThis.id + "_edit_field_" + column['field'] + '" type="text">');
					var comp = new $.compManager.plugs.float(compDiv[0],eOptions,viewModel);
				}else if(eType == 'datetime'){
					var compDiv = $('<input id="' + oThis.id + "_edit_field_" + column['field'] + '" type="text">');
					var comp = new $.compManager.plugs.datetime(compDiv[0],eOptions,viewModel);
				}else if(eType == 'date'){
...
//					if (comp.setDataTableRow)
//						comp.setDataTableRow(row)
...
//					});
			...
			if(rType == 'booleanRender'){
				...
				column.renderType = function(obj){
					...
					for( var i=0,length=ds.length; i<length; i++){
					   if(ds[i].pk==value){
					   	 obj.element.innerHTML = ds[i].name;
					   	 break;
					   }
					}					
				}
		...
		this.dataTable.on($.DataTable.ON_ROW_FOCUS, function(event) {
			...
			if(index > -1){
				oThis.grid.setRowFocus(parseInt(index));
			}
		});
		...
		this.dataTable.on($.DataTable.ON_ROW_UNFOCUS, function(event) {
			$.each(event.rowIds, function() {
				var index = oThis.grid.getRowIndexByValue('$_#_@_id',this);
				if(index > -1){
					oThis.grid.setRowUnFocus(parseInt(index));
				}
			});
		});
...
+function($ ) {
	var CheckboxComp = $.InputComp.extend({
		initialize: function(element, options, viewModel) {
			...
			this.create()
			if($.app){
			$(this.element).wrap("<label class='label-switch'></div>").after("<div class='checkbox'></div>").css("display","none")
    		}
...
+function($) {
	...
	var CheckboxGroupComp = $.InputComp.extend({
		...
		modelValueChange: function(val) {
			...
			val = val || ""
			this.trueValue = val
			var backstageVal = val.split(',')
			$(this.element).find(":checkbox").each(function() {
               for(var i=0;i<backstageVal.length;i++){
               	    if(this.value == backstageVal[i]){
               	    	this.checked = true;
               	    }
               }
			})
...
+function($) {
	...
	var DateTime = $.InputComp.extend({
		initialize: function(element, options, viewModel) {
			...
			this.options.format = "YYYY-M-D  HH:ss"; 
			...
			$(this.element).find('input').focusin(function(e) {
				self.setShowValue(self.getValue())
		...
		setShowValue : function(showValue) {
			...
			if($(this.element).data("DateTimePicker")){
				$(this.element).data("DateTimePicker").date(showValue)
			}	
		},
		getShowValue: function() {
			return this.showValue
		...
		setFormat: function(format){
 		...
 		setEnable: function(enable) {
			if (enable === true || enable === 'true') {
				...
				$(this.element).find("input").attr('h7picker','true')
				
			} else if (enable === false || enable === 'false') {
				this.enable = false
				$(this.element).find("input").attr('h7picker','false')
...
+function($) {
	...
	var Pagination = function(element, options, viewModel) {
		...
		this.$element.on('pageChange', function(event,pageIndex,pageSize) {
			var pagesize = parseInt($(event.target.lastChild).find(".page_z").val())
			if(!typeof(pagesize)){
				pagesize = options.pageSize
			}
			oThis.dataTable.cacheCurrentPage()
			viewModel['pageChange'](pageIndex-1,pagesize)  
		})
		...
		this.dataTable.pageIndex.subscribe(function(value) {
			oThis.pagination.update({currentPage:value})
		})
...
//		ko.cleanNode(this.element)
	...
	App.fn.compsValidate = function(element){
		...
		for(var i=0; i< comps.length; i++){
			...
			passed = comps[i].doValidate() && passed			
		}
		return passed
	}
